...
const StudentTimetablePage = () => {
  ...
  const fetchTimetable = useCallback(async (token: string, classId: number) => {
    try {
      const scheduleRes = await axios.get(`http://localhost:5000/api/schedules/class/${classId}?school_year=${schoolYear}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  useEffect(() => {
    ...
    const fetchStudentInfo = async () => {
        try {
            const res = await axios.get(`http://localhost:5000/api/auth/me?school_year=${schoolYear}`, { headers: { Authorization: `Bearer ${token}` } });
            if (!isMounted) return;
