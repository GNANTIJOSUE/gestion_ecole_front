...
const ParentDashboard = () => {
  ...
  useEffect(() => {
    const fetchChildren = async () => {
      ...
      try {
        const { data } = await axios.get(`http://localhost:5000/api/students?parent_code=${parent_code}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setChildren(data);
      } catch (e) {
    ...
    const fetchParent = async () => {
      const token = localStorage.getItem('token');
      try {
        const { data } = await axios.get(`http://localhost:5000/api/parents/me?school_year=${schoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setParent(data);
      } catch {
