...
const FinalizeRegistration = () => {
  ...
  const fetchRegistrations = useCallback(async () => {
    try {
      ...
      const { data } = await axios.get('https://schoolapp.sp-p6.com/api/students/online-registrations', {
  ...
  const fetchClasses = async () => {
    try {
      const token = localStorage.getItem('token');
      const { data } = await axios.get('https://schoolapp.sp-p6.com/api/classes', {
        headers: { Authorization: `Bearer ${token}` }
      });
      setClasses(data);
    } catch (err) {
  ...
  const handleFinalize = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`https://schoolapp.sp-p6.com/api/students/${selectedStudent.id}/finalize`, {
