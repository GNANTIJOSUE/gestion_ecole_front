      ...
      <Box component="main" sx={{ p: 3, flexGrow: 1, bgcolor: '#f0f7ff' }}>
        <Container maxWidth="lg">
          <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 5 }}>
            ...
            <Button variant="outlined" startIcon={<ArrowBackIcon />} onClick={() => navigate('/secretary/dashboard')}>
              Retour
            </Button>
          </Stack>
          ...
          {loading ? (
            ...
            <Grid container spacing={4}>
              {classes.map((cls) => {
                const color1 = stringToHslColor(cls.name, 45, 50);
                const color2 = stringToHslColor(cls.name, 60, 65);
                const gradient = `linear-gradient(135deg, ${color1} 0%, ${color2} 100%)`;
                const shadowHue = stringToHslColor(cls.name, 45, 50).match(/(\d+)/)?.[0] || '210';
                  ...
                  <Grid item xs={12} sm={6} md={4} key={cls.id}>
                    <Card sx={{
                      ...
                      '&:hover': {
                        ...
                        boxShadow: `0 16px 32px hsla(${shadowHue}, 40%, 40%, 0.3)`,
                      },
                    }}>
                      <Box sx={{ 
                          position: 'absolute', 
                          ...
                          <AssignmentIcon sx={{ fontSize: 140, transform: 'rotate(-15deg)' }}/>
                      </Box>
                      <CardActionArea 
                        onClick={() => navigate(`/secretary/timetables/${cls.id}`)}
                        sx={{ p: 3, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', minHeight: '190px', position: 'relative', zIndex: 1, height: '100%' }}
                      >
...
export default TimetableSelectionPage; 