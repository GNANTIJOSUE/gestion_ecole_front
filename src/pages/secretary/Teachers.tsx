...
const Teachers = () => {
  ...
  const fetchTeachers = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get('https://schoolapp.sp-p6.com/api/teachers', {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editModalOpen && selectedTeacher) {
        await axios.put(`https://schoolapp.sp-p6.com/api/teachers/${selectedTeacher.id}`, dataToSend, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        await axios.post('https://schoolapp.sp-p6.com/api/teachers', dataToSend, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
  ...
  const handleDeleteTeacher = async (teacherId: number) => {
    if (window.confirm('Voulez-vous vraiment supprimer ce professeur ?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://schoolapp.sp-p6.com/api/teachers/${teacherId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchTeachers();
      } catch (err) {
