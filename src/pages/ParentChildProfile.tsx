...
const PaiementDialog = ({ open, onClose, childId, schoolYear }: { open: boolean, onClose: () => void, childId: string | undefined, schoolYear: string }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [paymentsRes, studentRes] = await Promise.all([
          axios.get(`https://schoolapp.sp-p6.com/api/students/${childId}/payments?school_year=${schoolYear}`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`https://schoolapp.sp-p6.com/api/students/${childId}?school_year=${schoolYear}`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  const handleFusionPaySingle = async (payment: any) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post('https://schoolapp.sp-p6.com/api/payments', {
        student_id: childId,
...
const ParentChildProfile = () => {
  ...
  React.useEffect(() => {
    ...
    const fetchAnnualAverage = async () => {
      try {
        const token = localStorage.getItem('token');
        const { data } = await axios.get(`https://schoolapp.sp-p6.com/api/students/${childId}/annual-average?school_year=${schoolYear}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  React.useEffect(() => {
    ...
    const fetchPublished = async () => {
      ...
      await Promise.all(trimesters.map(async (trimester) => {
        try {
          const { data } = await axios.get(`https://schoolapp.sp-p6.com/api/report-cards/published?class_id=${studentClassId}&trimester=${encodeURIComponent(trimester)}&school_year=${schoolYear}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
